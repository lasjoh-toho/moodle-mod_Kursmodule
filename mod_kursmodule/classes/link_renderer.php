...
class link_renderer {
    ...
    public static function render_table(
        ...
        $out .= \html_writer::start_div('kursmodule-wrapper');
    ...
    protected static function inline_styles(): string {
        return '<style>' .
            // Wrapper: erzwingt einen eigenen Block ueber die volle Breite,
            // damit der "Verwalten"-Button auch dann sauber UNTER der
            // Tabelle landet, wenn das Kursformat die Aktivitaet selbst
            // per flex/inline darstellt.
            '.kursmodule-wrapper{display:block;width:100%;flex:1 1 100%;}' .
            '.kursmodule-table{display:flex;flex-direction:column;width:100%;' .
            'border:1px solid rgba(0,0,0,.3);border-radius:6px;overflow:hidden;margin-bottom:.5rem;}' .
            '.kursmodule-row{display:flex;align-items:stretch;text-decoration:none;color:inherit;' .
            'border-bottom:1px solid rgba(0,0,0,.3);background:transparent;' .
            'transition:background-color .12s ease;}' .
            '.kursmodule-row:last-child{border-bottom:none;}' .
            '.kursmodule-row:hover,.kursmodule-row:focus-visible{text-decoration:none;' .
            'background:rgba(47,111,235,.12);}' .
            '.kursmodule-row-img{width:120px;height:68px;object-fit:cover;flex-shrink:0;display:block;' .
            'margin:0;padding:0;border-right:1px solid rgba(0,0,0,.3);}' .
            ...
            '.kursmodule-managebutton{display:block;width:100%;clear:both;margin-top:.5rem;}' .
            '@media (prefers-color-scheme: dark){' .
            '.kursmodule-table,.kursmodule-row,.kursmodule-row-img,.kursmodule-row-img-placeholder{' .
            'border-color:rgba(255,255,255,.25);}' .
            '.kursmodule-row:hover,.kursmodule-row:focus-visible{background:rgba(79,140,255,.2);}' .
            '}' .
            '</style>';
    }
}
